...
class CrystalOptimizer(AOptimizer):
    ...
    def __init__(self, nn_model, loss_function):
        ...
        self._lower_bound, self._upper_bound = -2, 2
        self._nb_crystal = 15
        self._nb_iterations = 60
        seed = None
    ...
    def __compute_r_values(self, n: int = 2) -> (float, float, float, float):
        rnd_values = n * self._np.random(4)
    ...
    def _take_random_crystals(self, crystals: np.array, nb_random_crystals_to_take: int = 0) -> np.array:
        if nb_random_crystals_to_take <= 0:
            nb_random_crystals_to_take = np.random.randint(low=1, high=self._nb_crystal + 1)
        indexes_of_random_crystal = np.random.choice(range(0, self._nb_crystal), nb_random_crystals_to_take, replace=False)
    ...
    def _create_crystals(self, lb: int, ub: int) -> np.array:
        base_weights = self._flat_weights()
        dim = base_weights.size
        random_crystals = self._np.uniform(low=lb, high=ub, size=(self._nb_crystal, dim))
    ...
    def _optimize_impl(self, X_train: np.ndarray, y_train: np.ndarray) -> None:
        ...
        crystals = self._create_crystals(lb=self._lower_bound, ub=self._upper_bound)
        ...
        for i in range(0, self._nb_iterations):
            for crystal_idx in range(0, self._nb_crystal):
                new_crystals = np.array([])
                Cr_main = self._take_random_crystals(crystals=crystals, nb_random_crystals_to_take=1).flatten()
                Cr_old = crystals[crystal_idx]
                Fc = self._take_random_crystals(crystals=crystals).mean(axis=0)
                ...
                new_crystals = np.vstack((new_crystals, Cr_new))
                new_crystals = np.clip(new_crystals, a_min=self._lower_bound, a_max=self._upper_bound)
